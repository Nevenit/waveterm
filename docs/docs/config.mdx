...
| window:keepworkspacesresident        | bool     | keep other workspaces' tabs resident (off-screen) instead of destroying them on workspace switch, so switching back is instant. bounded by window:maxtabcachesize; to keep all workspaces warm, set that >= your total tab count. (defaults to false)          |
...
```json
{
    ...
    "window:keepworkspacesresident": false,
